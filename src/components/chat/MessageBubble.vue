...
<template>
  <div class="relative mb-3 flex w-fit max-w-[85%] flex-col md:max-w-[70%]"
    :class="[message.sender === 'visitor' ? 'self-end' : 'self-start']">
    ...
    <div v-else class="relative flex flex-col rounded-lg px-2 py-2 text-[15px] leading-relaxed shadow-sm" :class="[
      ...
      <span class="absolute top-0 w-2 h-3"
        :class="message.sender === 'visitor' ? '-right-2 text-wa-msg-out dark:text-wa-msg-out-dark' : '-left-2 text-wa-msg-in dark:text-wa-msg-in-dark'">
        <svg v-if="message.sender === 'visitor'" viewBox="0 0 8 13" width="8" height="13" fill="currentColor">
